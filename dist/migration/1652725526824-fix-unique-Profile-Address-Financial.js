"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.fixUniqueProfileAddressFinancial1652725526824 = void 0;
var fixUniqueProfileAddressFinancial1652725526824 = /** @class */ (function () {
    function fixUniqueProfileAddressFinancial1652725526824() {
        this.name = 'fixUniqueProfileAddressFinancial1652725526824';
    }
    fixUniqueProfileAddressFinancial1652725526824.prototype.up = function (queryRunner) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: 
                    //fix estadoId
                    return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDomicilio` DROP FOREIGN KEY `FK_4c805413f269f37d4892ec259ae`")];
                    case 1:
                        //fix estadoId
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("DROP INDEX `REL_4c805413f269f37d4892ec259a` ON `profileDomicilio`")];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDomicilio` ADD CONSTRAINT `FK_4c805413f269f37d4892ec259ae` FOREIGN KEY (`estadoId`) REFERENCES `staticEstadosMexico`(`idEstado`) ON DELETE NO ACTION ON UPDATE NO ACTION")];
                    case 3:
                        _a.sent();
                        //fix tipoDomicilioId
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDomicilio` DROP FOREIGN KEY `FK_c7419699e895f2db5a5c76358fc`")];
                    case 4:
                        //fix tipoDomicilioId
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("DROP INDEX `REL_c7419699e895f2db5a5c76358f` ON `profileDomicilio`")];
                    case 5:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDomicilio` ADD CONSTRAINT `FK_c7419699e895f2db5a5c76358fc` FOREIGN KEY (`tipoDomicilioId`) REFERENCES `staticTiposDomicilios`(`idTipoDomicilio`) ON DELETE NO ACTION ON UPDATE NO ACTION")];
                    case 6:
                        _a.sent();
                        //fix fuenteIngresoId
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` DROP FOREIGN KEY `FK_7711de54c8dc5eb1d4846e22969`")];
                    case 7:
                        //fix fuenteIngresoId
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("DROP INDEX `REL_7711de54c8dc5eb1d4846e2296` ON `profileDatosFinancieros`")];
                    case 8:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` ADD CONSTRAINT `FK_7711de54c8dc5eb1d4846e22969` FOREIGN KEY (`fuenteIngresoId`) REFERENCES `staticTiposIngresos`(`idTipoIngreso`) ON DELETE NO ACTION ON UPDATE NO ACTION")];
                    case 9:
                        _a.sent();
                        //fix industriaId
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` DROP FOREIGN KEY `FK_a876b8c218803db0b8e1442e8fa`")];
                    case 10:
                        //fix industriaId
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("DROP INDEX `REL_a876b8c218803db0b8e1442e8f` ON `profileDatosFinancieros`")];
                    case 11:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` ADD CONSTRAINT `FK_a876b8c218803db0b8e1442e8fa` FOREIGN KEY (`industriaLaboralId`) REFERENCES `staticIndustriasLaborales`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION")];
                    case 12:
                        _a.sent();
                        // updato to FE
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileUserInfo` DROP COLUMN `economicDependents`")];
                    case 13:
                        // updato to FE
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` DROP COLUMN `nombreComercial`")];
                    case 14:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` ADD `economicDependents` int NOT NULL")];
                    case 15:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    fixUniqueProfileAddressFinancial1652725526824.prototype.down = function (queryRunner) {
        return __awaiter(this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: 
                    //fix estadoId
                    return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDomicilio` DROP FOREIGN KEY `FK_4c805413f269f37d4892ec259ae`")];
                    case 1:
                        //fix estadoId
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("CREATE UNIQUE INDEX `REL_4c805413f269f37d4892ec259a` ON `profileDomicilio` (`estadoId`)")];
                    case 2:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDomicilio` ADD CONSTRAINT `FK_4c805413f269f37d4892ec259ae` FOREIGN KEY (`estadoId`) REFERENCES `staticEstadosMexico`(`idEstado`) ON DELETE NO ACTION ON UPDATE NO ACTION")];
                    case 3:
                        _a.sent();
                        //fix tipoDomicilioId
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDomicilio` DROP FOREIGN KEY `FK_c7419699e895f2db5a5c76358fc`")];
                    case 4:
                        //fix tipoDomicilioId
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("CREATE UNIQUE INDEX `REL_c7419699e895f2db5a5c76358f` ON `profileDomicilio` (`tipoDomicilioId`)")];
                    case 5:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDomicilio` ADD CONSTRAINT `FK_c7419699e895f2db5a5c76358fc` FOREIGN KEY (`tipoDomicilioId`) REFERENCES `staticTiposDomicilios`(`idTipoDomicilio`) ON DELETE NO ACTION ON UPDATE NO ACTION")];
                    case 6:
                        _a.sent();
                        //fix fuenteIngresoId
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` DROP FOREIGN KEY `FK_7711de54c8dc5eb1d4846e22969`")];
                    case 7:
                        //fix fuenteIngresoId
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("CREATE UNIQUE INDEX `REL_7711de54c8dc5eb1d4846e2296` ON `profileDatosFinancieros` (`fuenteIngresoId`)")];
                    case 8:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` ADD CONSTRAINT `FK_7711de54c8dc5eb1d4846e22969` FOREIGN KEY (`fuenteIngresoId`) REFERENCES `staticTiposIngresos`(`idTipoIngreso`) ON DELETE NO ACTION ON UPDATE NO ACTION")];
                    case 9:
                        _a.sent();
                        //fix industriaId
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` DROP FOREIGN KEY `FK_a876b8c218803db0b8e1442e8fa`")];
                    case 10:
                        //fix industriaId
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("CREATE UNIQUE INDEX `REL_a876b8c218803db0b8e1442e8f` ON `profileDatosFinancieros` (`industriaLaboralId`)")];
                    case 11:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` ADD CONSTRAINT `FK_a876b8c218803db0b8e1442e8fa` FOREIGN KEY (`industriaLaboralId`) REFERENCES `staticIndustriasLaborales`(`id`) ON DELETE NO ACTION ON UPDATE NO ACTION")];
                    case 12:
                        _a.sent();
                        // update to FE
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` DROP COLUMN `economicDependents`")];
                    case 13:
                        // update to FE
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileDatosFinancieros` ADD `nombreComercial` varchar(100) NULL")];
                    case 14:
                        _a.sent();
                        return [4 /*yield*/, queryRunner.query("ALTER TABLE `profileUserInfo` ADD `economicDependents` int NOT NULL")];
                    case 15:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    return fixUniqueProfileAddressFinancial1652725526824;
}());
exports.fixUniqueProfileAddressFinancial1652725526824 = fixUniqueProfileAddressFinancial1652725526824;
//# sourceMappingURL=1652725526824-fix-unique-Profile-Address-Financial.js.map